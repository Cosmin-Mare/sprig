	...
	if (admin.apps.length === 0) {
		return initializeApp({
			credential: admin.credential.cert(
				JSON.parse(
					Buffer.from(
						import.meta.env.FIREBASE_CREDENTIAL,
						"base64"
					).toString()
				)
			),
...
export const addDocument = async (
	...
	try {
		const data = await timedOperation(
			metricKey,
			async () => await firestore.collection(path).add(fields)
		);
...
export const getDocument = async (
	...
	try {
		const data = await timedOperation(
			metricKey,
			async () => await firestore.collection(path).doc(documentId).get()
		);
...
export const findDocument = async (
	path: string,
	where: WhereParam[] | [WhereParam, WhereFilterOp, WhereParam],
	limit: number = 1
): Promise<any> => {
	...
	try {
		...
		const data = await timedOperation(
			metricKey,
			async () => await collection.limit(limit).get()
		);
...
export const getSession = async (
	...
	const _session = await getDocument(
		"sessions",
		cookies.get("sprigSession").value!
	);
